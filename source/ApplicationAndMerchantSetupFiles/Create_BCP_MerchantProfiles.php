...
if ($_merchantProfileId == null) {
	...
	if (isset ( $_serviceId [0] )) {
		...
		$merchProfile = setBCPMerchantProfile ();
		$merchProfile->ProfileId = $merchProfile->ProfileId . '_' . $_serviceId ['singleService'] ['ServiceId'];
		$response = $client->saveMerchantProfiles ( $merchProfile, 'Credit', $_serviceId ['singleService'] ['ServiceId'] );
		if (is_array ( $response ))
			$response = arrayToObject ( $response );
	}
}
...
if ( is_array($response) && isset($response[1])) {
	foreach ( $response as $MerchantProfile ) {
		...
		printMerchantProfiles ( $MerchantProfile );
 ...
 */
function setBCPMerchantProfile() {
	// BankCard Merchant Data Object
	$bankCardMerchData = new BankcardMerchantData ();
	// <!--type: string--> Optional - 
	$bankCardMerchData->ABANumber = '';
	// <!--type: string--> Optional - 
	$bankCardMerchData->AgentChain = ''; // Was 128965
	// <!--type: string--> Optional - 
	$bankCardMerchData->AgentBank = '';
	// <!--type: string--> Optional -
	$bankCardMerchData->AcquirerBIN = '';
	// <!--type: string--> Optional - 
	$bankCardMerchData->Location = '';
	$bankCardMerchData->PrintCustomerServicePhone = 'false';
	// <!--type: string--> Optional - 
	$bankCardMerchData->ReimbursementAttribute = '';
	// <!--type: string--> Optional - 
	$bankCardMerchData->SettlementAgent = '';
	// <!--type: string--> Optional - 
	$bankCardMerchData->SharingGroup = '';
	// <!--type: string--> Optional - 
	$bankCardMerchData->StoreId = '';
	// <!--type: string--> Optional - 
	$bankCardMerchData->SecondaryTerminalId = '';
	// <!--type: string--> Optional - 
	$bankCardMerchData->TimeZoneDifferential = '';
	...
	$bankCardMerchData->SIC = '4599';
	// <!--type: string--> Optional - 
	$bankCardMerchData->ClientNumber = '';
	// <!--type: boolean-->
	$bankCardMerchData->Aggregator = Settings::Pro_IncludeAlternativeMerchantData;
	// <!--type: string-->
	$bankCardMerchData->TerminalId = '001';
	...
	// <!--type: string-->
	$merchantData->MerchantId = '123456789012';
	// <!--type: string-->
	...
	// <!--type: string-->
	$merchProfile->LastUpdated = '2012-07-24';
