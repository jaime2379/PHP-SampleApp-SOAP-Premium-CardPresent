...
if (is_array($_merchantProfileId)){
	foreach($_merchantProfileId as $profileId){
		...
		if (is_array($_bankcardServices->BankcardService)){
		...
		if($_bcs->Operations->Undo)
		{
			...
			$undDiffData = new BankcardUndo ();
			$undDiffData->PINDebitReason = 'NotSet';
			$undDiffData->TransactionCode = 'NotSet';
			$undDiffData->ForceVoid = false;
			$response4 = $client->undo($response3->TransactionId, $undDiffData, null, "BCP");
		...
		if($_bcs->Operations->ReturnById && !$_bcs->AutoBatch)
		{
			...
			else if ($_bcs->Operations->CaptureAll) {
				printTransactionResults($response3, 'Authorize For CaptureAll', $merchProfileId);
				$response4 = $client->CaptureAll(null, null);
			}
			else if ($_bcs->Operations->Capture) {
				printTransactionResults($response3, 'Authorize For Capture', $merchProfileId);
				$response4 = $client->Capture($response3->transactionId, $credentials, null, null);
			}
