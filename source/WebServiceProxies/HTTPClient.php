...
class HTTPClient {
		...
		if ($this->session_token == '') {
			try {
				$msgBody = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
								...
								'</s:Body>'.
							'</s:Envelope>';
				$action = 'http://schemas.evosnap.com/CWS/v2.0/ServiceInformation/ICWSServiceInformation/SignOnWithToken';
	...
	public function getServiceInformation() {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<GetServiceInformation xmlns="http://schemas.evosnap.com/CWS/v2.0/ServiceInformation">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'.
								'</GetServiceInformation>'.
							'</s:Body>'.
						'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/ServiceInformation/ICWSServiceInformation/GetServiceInformation';
	...
	public function getMerchantProfiles($svcId, $tndrType) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<GetMerchantProfiles xmlns="http://schemas.evosnap.com/CWS/v2.0/ServiceInformation">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'.
							...
							'</s:Body>'.
						'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/ServiceInformation/ICWSServiceInformation/GetMerchantProfiles';
			$response = curl_soap($msgBody, $this->svcInfo, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function getMerchantProfileId() {
		try{
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<GetMerchantProfileIds xmlns="http://schemas.evosnap.com/CWS/v2.0/ServiceInformation">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'.
							...
							'</s:Body>'.
						'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/ServiceInformation/ICWSServiceInformation/GetMerchantProfileIds';
			$response = curl_soap($msgBody, $this->svcInfo, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function isMerchantProfileInitialized($merchProfileId, $serviceId) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<IsMerchantProfileInitialized xmlns="http://schemas.evosnap.com/CWS/v2.0/ServiceInformation">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'.
									'<serviceId>'.$serviceId.'</serviceId>'.
							...
							'</s:Body>'.
						'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/ServiceInformation/ICWSServiceInformation/IsMerchantProfileInitialized';
			$response = curl_soap($msgBody, $this->svcInfo, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function saveApplicationData($appData) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<SaveApplicationData xmlns="http://schemas.evosnap.com/CWS/v2.0/ServiceInformation">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'.
			$appData.
								'</SaveApplicationData>'.
							'</s:Body>'.
						'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/ServiceInformation/ICWSServiceInformation/SaveApplicationData';
			$response = curl_soap($msgBody, $this->svcInfo, $action);
	...
	public function saveMerchantProfiles($merchantProfile, $tenderType, $serviceId) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<SaveMerchantProfiles xmlns="http://schemas.evosnap.com/CWS/v2.0/ServiceInformation">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'.
									'<serviceId>'.$serviceId.'</serviceId>'.
									'<tenderType>'.$tenderType.'</tenderType>'.
			...
			$merchantProfile.
							...
							'</s:Body>'.
						'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/ServiceInformation/ICWSServiceInformation/SaveMerchantProfiles';
			$response = curl_soap($msgBody, $this->svcInfo, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function authorize($credit_info, $trans_info, $processAsPro = false) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<Authorize xmlns="http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'
									.$transaction.
							...
							'</s:Body>'.
							'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing/ICwsTransactionProcessing/Authorize';
	...
	public function verify($trans_info) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<Verify xmlns="http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'.
			$trans_info.
									'<applicationProfileId>'.$this->appProfileID.'</applicationProfileId>'.
									'<merchantProfileId>'.$this->merchantProfileID.'</merchantProfileId>'.
							...
							'</s:Body>'.
							'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing/ICwsTransactionProcessing/Verify';
			$response = curl_soap($msgBody, $this->txn, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function capture($transactionID, $creds = null, $amount = null, $tip_amount = null) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<Capture xmlns="http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'
									.$capDiffXML.
									'<applicationProfileId>'.$this->appProfileID.'</applicationProfileId>'.									
									'<workflowId>'.$this->workflowId.'</workflowId>'.
							...
							'</s:Body>'.
							'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing/ICwsTransactionProcessing/Capture';
			$response = curl_soap($msgBody, $this->txn, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function authorizeAndCapture($credit_info, $trans_info, $processAsPro) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							...
							'</s:Body>'.
							'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing/ICwsTransactionProcessing/AuthorizeAndCapture';
			$response = curl_soap($msgBody, $this->txn, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function undo( $transactionID, $creds = null, $txnType = null ) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<Undo xmlns="http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'
									.$trans_info.
									'<applicationProfileId>'.$this->appProfileID.'</applicationProfileId>'.									
									'<workflowId>'.$this->workflowId.'</workflowId>'.
							...
							'</s:Body>'.
							'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing/ICwsTransactionProcessing/Undo';
			$response = curl_soap($msgBody, $this->txn, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function returnByID( $transactionID, $creds = null, $amount = null) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<ReturnById xmlns="http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'
									.$trans_info.
									'<applicationProfileId>'.$this->appProfileID.'</applicationProfileId>'.									
									'<workflowId>'.$this->workflowId.'</workflowId>'.
							...
							'</s:Body>'.
							'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing/ICwsTransactionProcessing/ReturnById';
			$response = curl_soap($msgBody, $this->txn, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function returnUnlinked( $credit_info, $trans_info) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<ReturnUnlinked xmlns="http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'
									.$transaction.
									'<applicationProfileId>'.$this->appProfileID.'</applicationProfileId>'.
									'<merchantProfileId>'.$this->merchantProfileID.'</merchantProfileId>'.
							...
							'</s:Body>'.
							'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing/ICwsTransactionProcessing/ReturnUnlinked';
			$response = curl_soap($msgBody, $this->txn, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function captureSelective($transactionIds, $differenceData, $creds = null) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<CaptureSelective xmlns="http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'.
			$transactionIds.
			$differenceData.
									'<applicationProfileId>'.$this->appProfileID.'</applicationProfileId>'.
							...
							'</s:Body>'.
							'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing/ICwsTransactionProcessing/CaptureSelective';
			$response = curl_soap($msgBody, $this->txn, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function captureAll($differenceData, $creds = null) {
		...
		try {
			$msgBody = 	'<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">'.
							'<s:Body>'.
								'<CaptureAll xmlns="http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing">'.
									'<sessionToken>'.$this->session_token.'</sessionToken>'.
			$differenceData.
									'<applicationProfileId>'.$this->appProfileID.'</applicationProfileId>'.
									'<merchantProfileId>'.$this->merchantProfileID.'</merchantProfileId>'.
							...
							'</s:Body>'.
							'</s:Envelope>';							
			$action = 'http://schemas.evosnap.com/CWS/v2.0/TransactionProcessing/ICwsTransactionProcessing/CaptureAll';
			$response = curl_soap($msgBody, $this->txn, $action);
			if (isset($response['Body']['Fault'])){
				$xmlFault = generate_valid_xml_from_array($response);
				throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function queryTransactionsSummary($queryTransactionParameters, $includeRelated, $pagingParameters) {
		...
		try{
			$msgBody = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">';
			$msgBody .= '<s:Body>';
			$msgBody .= '<QueryTransactionsSummary xmlns="http://schemas.evosnap.com/CWS/v2.0/DataServices/TMS">';
			...
			if ($queryTransactionParameters->ApprovalCodes != null){
				...
				if ($queryTransactionParameters->BatchIds != null){
					...
					if ($queryTransactionParameters->CaptureDateRange != null){
						$msgBody .= '<CaptureDateRange xmlns:a="http://schemas.evosnap.com/CWS/v2.0/DataServices">'.$queryTransactionParameters->CaptureDateRange.'</CaptureDateRange>';}
						if ($queryTransactionParameters->CaptureStates != null){
							$msgBody .= '<CaptureStates xmlns:a="http://schemas.evosnap.com/CWS/v2.0/Transactions">';
							...
							if ($queryTransactionParameters->CardTypes != null){
								$msgBody .= '<CardTypes xmlns:a="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">';
										...
										if ($queryTransactionParameters->ServiceKeys != null){
											...
											if ($queryTransactionParameters->TransactionClassTypePairs != null){
												...
												if(isset($queryTransactionParameters->TransactionDateRange->StartDateTime)){
													$msgBody .= '<TransactionDateRange xmlns:a="http://schemas.evosnap.com/CWS/v2.0/DataServices">';
												...
												if ($queryTransactionParameters->TransactionIds != null){
													...
													if ($queryTransactionParameters->TransactionStates != null){
														$msgBody .= '<TransactionStates xmlns:a="http://schemas.evosnap.com/CWS/v2.0/Transactions">';
														...
														$msgBody .= '<pagingParameters xmlns:a="http://schemas.evosnap.com/CWS/v2.0/DataServices" xmlns:i="http://www.w3.org/2001/XMLSchema-instance">';
														...
														$msgBody .= '</s:Envelope>';
														echo ($msgBody);
														$action =  'http://schemas.evosnap.com/CWS/v2.0/DataServices/TMS/ITMSOperations/QueryTransactionsSummary';
	...
	public function queryTransactionFamilies($queryTransactionParameters, $includeRelated, $pagingParameters) {
		...
		try{
			$msgBody = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">';
			$msgBody .= '<s:Body>';
			$msgBody .= '<QueryTransactionFamilies xmlns="http://schemas.evosnap.com/CWS/v2.0/DataServices/TMS">';
			$msgBody .= '<sessionToken>'.$this->session_token.'</sessionToken>';
			$msgBody .= '<queryTransactionsParameters xmlns:i="http://www.w3.org/2001/XMLSchema-instance">';
			if ($queryTransactionParameters->Amounts != null){
				$msgBody .= '<Amounts xmlns:a="http://schemas.microsoft.com/2003/10/Serialization/Arrays">';
											...
											if ($queryTransactionParameters->TransactionClassTypePairs != null){
												...
												if ($queryTransactionParameters->TransactionIds != null){
													...
													if ($queryTransactionParameters->TransactionStates != null){
														...
														$msgBody .= '</s:Body>';
														$msgBody .= '</s:Envelope>';
														$action =  'http://schemas.evosnap.com/CWS/v2.0/DataServices/TMS/ITMSOperations/QueryTransactionFamilies';
														$response = curl_soap($msgBody, $this->tms, $action);
														if (isset($response['Body']['Fault'])){
															$xmlFault = generate_valid_xml_from_array($response);
															throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
	...
	public function queryTransactionsDetail($queryTransactionParameters, $includeRelated, $transactionDetailFormat, $pagingParameters) {
		...
		try{
			$msgBody = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">';
			$msgBody .= '<s:Body>';
			$msgBody .= '<QueryTransactionsDetail xmlns="http://schemas.evosnap.com/CWS/v2.0/DataServices/TMS">';
			$msgBody .= '<sessionToken>'.$this->session_token.'</sessionToken>';
			$msgBody .= '<queryTransactionsParameters xmlns:i="http://www.w3.org/2001/XMLSchema-instance">';
			if ($queryTransactionParameters->Amounts != null){
				$msgBody .= '<Amounts xmlns:a="http://schemas.microsoft.com/2003/10/Serialization/Arrays">';
											...
											if ($queryTransactionParameters->TransactionClassTypePairs != null){
												...
												if ($queryTransactionParameters->TransactionIds != null){
													...
													if ($queryTransactionParameters->TransactionStates != null){
														...
														$msgBody .= '</s:Body>';
														$msgBody .= '</s:Envelope>';
														$action =  'http://schemas.evosnap.com/CWS/v2.0/DataServices/TMS/ITMSOperations/QueryTransactionsDetail';
														$response = curl_soap($msgBody, $this->tms, $action);
														if (isset($response['Body']['Fault'])){
															$xmlFault = generate_valid_xml_from_array($response);
															throw new SoapFault($response['Body']['Fault']['faultcode'], $response['Body']['Fault']['faultstring'], null, $response['Body']['Fault']['detail'], $xmlFault);
...
public function buildTransactionXML($credit_info, $trans_info) {
	...
	$trans = $trans.'<ApplicationConfigurationData i:nil="true" xmlns="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard"/>'.
				'<TenderData xmlns="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">';	
...
public function buildCaptureXML($captureDiffData, $creds = ''){
	$capture = '<differenceData i:type="b:BankcardCapturePro" xmlns:a="http://schemas.evosnap.com/CWS/v2.0/Transactions" xmlns:i="http://www.w3.org/2001/XMLSchema-instance" xmlns:b="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard/Pro">';
	...
	$capture = $capture.'<Amount xmlns="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">'.$captureDiffData->Amount.'</Amount>';
	if (isset($captureDiffData->ChargeType))
	$capture = $capture.'<ChargeType xmlns="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">'.$captureDiffData->ChargeType.'</ChargeType>';
	if (isset($captureDiffData->ShipDate))
	$capture = $capture.'<ShipDate xmlns="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">'.$captureDiffData->ShipDate.'</ShipDate>';
	if (isset($captureDiffData->TipAmount))
	$capture = $capture.'<TipAmount xmlns="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">'.$captureDiffData->TipAmount.'</TipAmount>';
	...
	$capture = $capture.'<b:Level2Data i:nil="true" xmlns:c="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard"/>'.
				'<b:LineItemDetails i:nil="true" xmlns:c="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard"/>'.
...
public function buildCaptureSelectiveXML($captureDiffData, $creds = ''){
	$capture = '<differenceData xmlns:a="http://schemas.evosnap.com/CWS/v2.0/Transactions" xmlns:i="http://www.w3.org/2001/XMLSchema-instance">';
	$i = 0;
	$count = count($captureDiffData->TransactionId);
	if (is_array($captureDiffData->TransactionId)){
		for ($i = 0; $i < $count ; $i++){
			if ($captureDiffData->TransactionId[$i] != null){
				$capture = $capture.'<a:Capture i:type="b:BankcardCapture" xmlns:b="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">'.
...
public function buildReturnByIdXML($returnDiffData, $creds = ''){
	...
	$return = '<differenceData i:type="b:BankcardReturn" xmlns:a="http://schemas.evosnap.com/CWS/v2.0/Transactions" xmlns:i="http://www.w3.org/2001/XMLSchema-instance" xmlns:b="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">'.
	...
	$return = $return.'<a:TransactionDateTime>'.$returnDiffData->TransactionDateTime.'</a:TransactionDateTime>'.
				'<Amount xmlns="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">'.$returnDiffData->Amount.'</Amount>'.
			'</differenceData>';
	return $return;
}
...
public function buildUndoXML($undoDiffData, $creds = ''){
	...
	$undo = '<differenceData i:type="b:BankcardUndo" xmlns:a="http://schemas.evosnap.com/CWS/v2.0/Transactions" xmlns:i="http://www.w3.org/2001/XMLSchema-instance" xmlns:b="http://schemas.evosnap.com/CWS/v2.0/Transactions/Bankcard">'.
